...
class HeartBtn extends React.Component {
  ...
  render() {
    ...
    return (
      <button>
        <svg className={this.state.btn_color ? 'off_button' : 'on_button'} width={this.props.width} height={this.props.height} viewBox='0 0 48 48' onClick={this.btnChangeHandler}>
          <path d={this.state.btn_color ? path_heart_empty : path_heart_fill} />
        </svg>
      </button>
    );
  }
